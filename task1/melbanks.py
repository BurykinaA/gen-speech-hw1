...
class LogMelFilterBanks(nn.Module):
    ...
    def forward(self, x):
        ...
        # [batch, n_fft//2 + 1, time]
        x = self.spectrogram(x)  
        
        # magnitude if power=1, power spectrum if power=2
        x = x.abs().pow(self.power)
        
        # [batch, time, freq] @ [freq, n_mels] -> [batch, time, n_mels]
        x = torch.einsum('...ft,fm->...tm', x, self.mel_fbanks)
        
        # [batch, n_mels, time]
        x = torch.log(x.transpose(-1, -2) + 1e-6)
